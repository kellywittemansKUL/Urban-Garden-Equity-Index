...
```{r}
#If needed you can run on parrallel
#if parallel 
#n_cores <- 5  
...
temp <- tempfile()
tempd <- tempdir()
tempd <- paste0("./Input/BBK/", BBK,"/")
rasters1 <- list.files(path=file.path(tempd),  pattern="\\.tif$")
directory1 <- file.path(tempd)
unlink(tempd) 

#administrative parcel dataset (building + garden) 
adp_0 <- st_read("./Input/Adp/Adp.shp") 
adp_1 <- st_read("./Adp/Adp_1.shp") 
adp <- rbind(adp_0, adp_1)
rm(adp_0, adp_1)

#Topographic Wetness Index (TWI)
TWI <- raster('./Input/TWI/TWI_Quinn_vlaanderen_filled_sinksPlanchon.tif')
#Q3 bepalen
...
#TWI <- raster('./Input/TWI/TWI_Flanders_clipped.tif')
#q_low_TWI <- quantile(TWI, 0.05, na.rm = TRUE)
#q_high_TWI <- quantile(TWI, 0.95, na.rm = TRUE)

#Land Surface Temperature
LST <- raster('./Input/LST/LST_Flanders.tif')
LST[LST < 0] <- NA
...
#LST <- raster('./Input/LST/LST_Flanders_clipped.tif')
...
Vulnerability <- st_read('./Input/gardens_vulnerability_quantile_scaled_Flanders.shp')

#Exposure dataset
exposure <- raster('./Input/Pop/ni_inw_ha_vlaa_2019.tif')
...
#exposure <- raster('./Input/Pop/ni_inw_ha_vlaa_2013.tif')
...
```{r}
#Garden map
tuinkaarten <- list.files(path=file.path("./Input/Buffers"),  pattern="\\.gpkg$")
directory2 <- file.path("./Input/Buffers")
...
for(i in 1:length(tuinkaarten)){
  ...
  gbg <- st_read(paste0("./Input/Gbg/", cities,"/gbg.shp")) #building vector map 
  
  #not needed for grouped gardens
  #tuinenkaart <- tuinenkaart %>%
  #filter(LGK %in% c("Huizen en Tuinen", "Overige bebouwde terreinen", "Overige onbebouwde terreinen")) #filter garden parcel map 
